...
getTimer()
    ...
    function countDown(data) {
        ...
        if (distance > 0) {
            ...
            nano = Math.floor((distance % (1000 * 60)) / 10).toString().slice(-2);
        }
    ...
    function getAnnouncements() {
        fetch('/announcement').then(res => res.json())
            .then(data => announcements = data.announcements)
    }
...
<div class="overflow-hidden">
    ...
    <h1 class="text-white break-word">Time: {announcement}</h1>
    {/each}
    </div>
    ...
    <h1 class="text-white break-word">{order + 1}#_{team}</h1>
    {/each}
    </div>
